...
class ClockPanel extends JPanel implements Runnable {
  ...
  private final int HORIZONTAL_SIZE = 100;
  private final int VERTICAL_SIZE = 100;

  /** The length of the clock hands relative to the clock size. */
  private final int secondHandLength = HORIZONTAL_SIZE / 2 - 10;
  private final int minuteHandLength = HORIZONTAL_SIZE / 2 - 14;
  private final int hourHandLength = HORIZONTAL_SIZE / 2 - 20;

  /** The distance of the dots from the origin (center of the clock). */
  private final int DISTANCE_DOT_FROM_ORIGIN = HORIZONTAL_SIZE / 2 - 8;

  private final int DIAMETER_BIG_DOT = 4;
...
*/
  ...
  protected void paintComponent(Graphics g){
    ...
    for (int i = 0; i < 60; i++) {
      ...
      g2.setColor((i <= Calendar.getInstance().get(Calendar.SECOND)) ? Color.RED : Color.BLUE);
